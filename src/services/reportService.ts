...
export const exportPdf = async (options: ExportOptions) => {
  try {
    ...
    const orientation = options.formatting.orientation === 'landscape' ? 'landscape' : 'portrait';
    const doc = new jsPDF({
      orientation,
      unit: 'mm',
    ...
    const headerText = options.formatting.headerText.replace('{{date}}', format(new Date(), 'dd/MM/yyyy'));
    doc.setFontSize(12);
    ...
    const periodText = `Período: ${format(from, 'dd/MM/yyyy', { locale: ptBR })} a ${format(to, 'dd/MM/yyyy', { locale: ptBR })}`;
    doc.setFontSize(10);
    doc.text(periodText, 14, 26);
    
    // Totalizadores
    const totalSales = sales?.length || 0;
    const totalValue = sales?.reduce((sum, sale) => sum + (sale.total || 0), 0) || 0;
    
    doc.setFontSize(10);
    doc.text(`Total de vendas: ${totalSales}`, 14, 32);
    doc.text(`Valor total: ${new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(totalValue)}`, 14, 37);
    ...
    const tableData = sales?.map(sale => {
      ...
      columns.forEach(column => {
        if (column === 'date') {
          row.push(format(new Date(sale[column]), 'dd/MM/yyyy', { locale: ptBR }));
        } else if (column === 'total') {
          row.push(new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(sale[column] || 0));
    ...
    const footerText = options.formatting.footerText.replace('{{date}}', format(new Date(), 'dd/MM/yyyy HH:mm', { locale: ptBR }));
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(text, pageWidth - textWidth - 14, doc.internal.pageSize.getHeight() - 10);
    }
