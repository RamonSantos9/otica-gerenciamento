...
export const formatCustomerFromSupabase = (customer: any): Customer => ({
  ...
  lastPurchase: customer.last_purchase
    ? new Date(customer.last_purchase).toISOString()
    : "",
...
export const formatProductFromSupabase = (product: any): Product => ({
  ...
  lastRestock: product.last_restock
    ? new Date(product.last_restock).toISOString()
    : "",
});
...
export const formatSaleFromSupabase = async (sale: any): Promise<Sale> => {
  ...
  const formattedItems: SaleItem[] = items
    ? items.map((item) => ({
